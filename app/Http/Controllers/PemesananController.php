...
public function updateSuccess(Request $request)
{
    ...
    public function riwayat()
    {
        ...
        $sudahDibayar = \App\Models\Pemesanan::with(['lapangan', 'jadwal'])
            ->where('penyewa_id', $userId)
            ->where('status', 'dibayar')
            ->get();

        $batal = \App\Models\Pemesanan::with(['lapangan', 'jadwal'])
            ->where('penyewa_id', $userId)
            ->where('status', 'batal')
            ->get();

        $scan = \App\Models\Pemesanan::with(['lapangan', 'jadwal'])
            ->where('penyewa_id', $userId)
            ->where('status', 'di-scan')
            ->get();

        return view('penyewa.riwayat', compact('sudahDibayar', 'batal', 'scan'));
    }
}