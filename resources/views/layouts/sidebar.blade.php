        ...
        <nav class="menu-list">
            @foreach ($menuItems as $item)
                @if (isset($item['submenu']))
                    ...
                    {{-- Dropdown --}}
                    <div class="menu-item">
                        <div class="menu-link dropdown-toggle {{ $isParentActive ? 'active' : '' }}"
                        ...
                        <div class="submenu {{ $isParentActive ? 'show' : '' }}">
                            @foreach ($item['submenu'] as $sub)
                                @php
                                    $isActive =
                                        isset($sub['active_routes']) &&
                                        Route::currentRouteNamed(...$sub['active_routes']);
                                    $url = Route::has($sub['route']) ? route($sub['route']) : '#';
                                @endphp
                                <a href="{{ $url }}"
                                    class="{{ $isActive ? 'active' : '' }}">{{ $sub['label'] }}</a>
                            @endforeach
                        </div>
                    </div>
                @else
    ...
    <script>
        ...
        dropdownToggles.forEach(toggle => {
            toggle.addEventListener('click', () => {
                ...
                document.querySelectorAll('.menu-link.dropdown-toggle').forEach(l => l.classList.remove(
                    'active'));
